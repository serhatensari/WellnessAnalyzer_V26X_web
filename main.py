...
ÖZEL DURUMLAR:
- Eğer marka 'Genel wellness analizi (markasız)' ise:
  marka ismi kullanma, sadece ürün/etki tipi yaz.
- Eğer marka OneMore dışındaki bir marka ise:
  OneMore ürünlerini ASLA yazma.
...
- Her üründe:
  - 'urun': Aşağıdaki ürün listesinden birebir isim.
  - 'neden': Eski ve yeni test arasındaki değişime göre en az 2 cümlelik açıklama.
  - 'sure': Örn. '4 hafta', '8 hafta'.
...
def apply_brand_product_filter(analysis: dict, brand: str) -> dict:
    ...
    - Marka ürün listesinde olmayan ürün isimlerini atar.
    - 'generic' ve 'pati' markalarında isim temizliği hariç sert filtre uygulamaz.
    ...
    if isinstance(gb, dict):
        gb["onemore_urun_onerileri"] = _fix_list(gb.get("onemore_urun_onerileri", []))

    # KANALLAR
    kk = analysis.get("kanallar_ve_kollateraller_detay")
    if isinstance(kk, dict):
        kk["onemore_urun_onerileri"] = _fix_list(kk.get("onemore_urun_onerileri", []))

    # BİLİNÇ
    ib = analysis.get("insan_bilinc_duzeyi_detay")
    if isinstance(ib, dict):
        ib["onemore_urun_onerileri"] = _fix_list(ib.get("onemore_urun_onerileri", []))
    ...
    if isinstance(cards, list):
        for kart in cards:
            if not isinstance(kart, dict):
                continue
            kart["urun_onerileri"] = _fix_list(kart.get("urun_onerileri", []))

    # ANALYSIS GENEL
    if "onemore_urun_onerileri" in analysis:
        analysis["onemore_urun_onerileri"] = _fix_list(analysis.get("onemore_urun_onerileri", []))
